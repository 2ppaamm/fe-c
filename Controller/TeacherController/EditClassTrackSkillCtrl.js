(function () {
    ...
    angular
        ...
        .controller('EditClassTrackSkillCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth', 'store', 'TeacherService','$location', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store, TeacherService,$location) {
			...
			if(store.get('GetDashboard') == undefined){
				TeacherService.GetDashboard().then(function(response){
					...
					var cca = response.enrolled_classes;
						...
						if (classid == houses[h].id) {
							...
							for (var t = 0; t < houses[h].tracks.length; t++) {
								if (trackid == houses[h].tracks[t].id) {
									...
									for (var s = 0; s < houses[h].tracks[t].skills.length; s++) {
										if (skillid == houses[h].tracks[t].skills[s].id) {
											skillname = houses[h].tracks[t].skills[s].skill;
											sdescription = houses[h].tracks[t].skills[s].description;
										}
									}
								}
							}
