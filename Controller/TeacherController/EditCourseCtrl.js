(function () {
    ...
    angular
        ...
        .controller('EditCourseCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth', 'store', 'TeacherService','$location', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store, TeacherService,$location) {
			...
			if(store.get('GetDashboard') == undefined){
				TeacherService.GetDashboard().then(function(response){
					...
					for (var c = 0; c < course.length; c++) {
						if (course[c].id == courseid) {
							var coursename = course[c].course;
							var description = course[c].description;

						}
					}
					$scope.cn = coursename;
					$scope.dp = description;
					$scope.coursename = coursename;
					$scope.description = description;
					$scope.cname = coursename;
            ...
            $scope.updatecourse = function () {
                ...
                if ($scope.coursename == undefined) {
                    ...
                    if ($scope.description == undefined) {
                        ...
                        TeacherService.UpdateCourse(courseid, cn, dp).then(function () {
                            alert('Successfully update course information !');
							store.remove('GetDashboard');
							TeacherService.GetDashboard().then(function(response){
