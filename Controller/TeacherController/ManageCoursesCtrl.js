        ...
        .controller('ManageCoursesCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth', 'store', 'TeacherService', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store, TeacherService) {
			...
			if(store.get('GetDashboard') == undefined){
				TeacherService.GetDashboard().then(function(response){
					...
					var status = "";
					var sid = 0;
					for (var c = 0; c < acourses.length; c++) { 
						if (acourses[c].status_id == 1) {
							status = "Only Me";
							sid = 1;
						}
						else {
							if (acourses[c].status_id == 2) {
								status = "Restricted";
								sid = 1;
							}
							else
							{
								if (acourses[c].status_id == 3) {
									status = "Public";
									sid = 0;
								}
								else {
									if (acourses[c].status_id == 4) {
										status = "Draft";
										sid = 1;
									}
								}
						...
						courses.push({
							number: num += 1,
							id:acourses[c].id,
							cname: acourses[c].course,
							cdescription: acourses[c].description,
							tracks: acourses[c].tracks,
							status: status,
							sid: sid
						});
					}
            ...
            $scope.trackongo = function (cid) {
                $window.location.href = $scope.tmcot + "?" + cid;
            }

            $scope.addcourseongo = function () {
                $window.location.href = $scope.taco;
            }

            $scope.editcourse = function (cid) {
                $window.location.href = $scope.teco + "?" + cid;
            }

            $scope.deletecourse = function (cid) {
                TeacherService.DeleteCourse(cid).then(function () {
                    ...
                    alert('Error occur, unsuccessfully delete course !');
                });
            }
            ...
            $scope.publish = function (cid) {
                TeacherService.PublishCourse(cid).then(function () {
                    alert('Successfully publish this course !');
					store.remove('GetDashboard');
					TeacherService.GetDashboard().then(function(response){
						store.set('GetDashboard',response.data);
					});
                    ...
                    alert('Error occur when publish the course, please contact admin !');
                });
            }
        }]);
})();