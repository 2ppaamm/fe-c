(function () {
    ...
    angular
        ...
        .controller('EditClassCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth', 'store', 'TeacherService','$location', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store, TeacherService,$location) {
				...
				TeacherService.GetDashboard().then(function(response){
					...
					for (var i = 0; i < classes.length; i++) {
						if (classid == classes[i].id) {
							classname = classes[i].house;
							courseid = classes[i].course_id;
							cdescription = classes[i].description;
							cstart = classes[i].start_date;
							cend = classes[i].end_date;
					...
					$scope.cname = classname;
					$scope.courses = coursename;
					$scope.classname = classname;
					$scope.description = cdescription;
					$scope.cstart = cstart;
					$scope.cend = cend;
					$scope.courses2 = coursename;
					$scope.classname2 = classname;
					$scope.description2 = cdescription;
					$scope.cstart2 = cstart;
					$scope.cend2 = cend;
            ...
            $scope.reset = function () {
                var cn = $scope.courses2;
                var cln = $scope.classname2;
                var cdp = $scope.description2;
                var cs = $scope.cstart2;
                var ce = $scope.cend2;
                $scope.courses = cn;
                $scope.classname = cln;
                $scope.description = cdp;
                $scope.cstart = cs;
                $scope.cend = ce;
            }
            ...
            $scope.empty = function () {
                ...
                $scope.cstart = undefined;
                $scope.cend = undefined;
            }
        }]);
})();