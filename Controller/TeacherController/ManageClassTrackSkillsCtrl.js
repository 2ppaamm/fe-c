(function () {
    ...
    angular
        ...
        .controller('ManageClassTrackSkillsCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth', 'store', 'TeacherService','$location', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store, TeacherService,$location) {
            ...
            var cid = url[7].split('?');
            var cidc = cid[1].split('@');
            var classid = cidc[0];
            var trackid = cidc[1];

            $scope.classname = "";
            $scope.trackname = "";
            $scope.skillinformation = [];
			...
			if(store.get('GetDashboard') == undefined){
				TeacherService.GetDashboard().then(function(response){
					...
					var cntn = response.houses;
					var cname = "";
					var tname = "";
					for (var c = 0; c < cntn.length; c++) {
						if (cntn[c].id == classid) {
							cname = cntn[c].house;
						}
						for (var t = 0; t < cntn[c].tracks.length; t++) {
							if (cntn[c].tracks[t].id == trackid) {
								tname = cntn[c].tracks[t].track;
							}
						}
					}
            ...
            var onSkillLoadComplete = function (response) {
                ...
                for (var s = 0; s < skills.length; s++) {
                    sk.push({
                        id: skills[s].id,
                        number: num += 1,
                        sname: skills[s].skill,
                        sdescription: skills[s].description
                    })
                }
                $scope.skillinformation = sk;
                loading_screen.finish();
            }

            // load all data once reeady for the page
            TeacherService.GetReadAllTrackSkill(trackid).then(onSkillLoadComplete);
            ...
            $scope.questionongo = function (sid) {
                $window.location.href = $scope.tmcltsq + "?" + classid + "@" + trackid + "-" + sid;
            }
            ...
            $scope.addskillongo = function () {
                $window.location.href = $scope.taclts + "?" + classid + "@" + trackid;
            }

            $scope.editskillongo = function (sid) {
                $window.location.href = $scope.teclts + "?" + classid + "@" + trackid + "-" + sid;
            }

            $scope.deleteskillongo = function (sid) {
                TeacherService.DeleteTrackSkill(trackid, sid).then(function () {
                    alert('Skill has been deleted !');
                    $window.location.href = $scope.tmclts + "?" + classid + "@" + trackid;
                }, function (err) {
                    alert('Something went wrong, unsuccessfull delete skill, please contact admin !');
                });
            }
