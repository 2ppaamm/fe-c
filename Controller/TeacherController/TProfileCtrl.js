(function () {
    ...
    angular
        ...
        .controller('TProfileCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth', 'store', 'TeacherService', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store,TeacherService) {
            ...
            $scope.updateownprofile = function () {
                var uid = $scope.uid;
                var fname = $scope.fname;
                var lname = $scope.lname;
                var contact = $scope.contact;

                TeacherService.PutUpdateProfile(uid, fname, lname, contact).then(function () {
                    alert('Successfully update user profile information!');
					store.remove('GetDashboard');
					TeacherService.GetDashboard().then(function(response){
						store.set('GetDashboard',response);
					});
                    $window.location.href = $scope.tpf;
                }, function (err) {
                    alert('Error occur when update user information, please contact admin!');
                });
            }
        }]);
})();