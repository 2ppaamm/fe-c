(function () {
    ...
    angular
        ...
        .controller('AddCourseTrackSkillCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth', 'store', 'TeacherService','$location', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store, TeacherService,$location) {
					...
					for (var c = 0; c < courses.length; c++) {
						if (courseid == courses[c].id) {
							...
							for (var t = 0; t < courses[c].tracks.length; t++) {
								console.log(courses[c].tracks[t]);
								if (courses[c].tracks[t].id == trackid) {
            ...
            $scope.addnewcoursetrack = function () {
                ...
                TeacherService.PostCreateTrackSkill(trackid, sk, sd).then(function () {
                    alert('Successfull create a new skill for the track !');
					store.remove('GetDashboard');
					TeacherService.GetDashboard().then(function(response){
                    ...
                    if ($scope.skillname == undefined) {
                        alert('Skill name field is required !');
                    }
                    else {
