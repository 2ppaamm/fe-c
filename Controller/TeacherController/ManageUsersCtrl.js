        ...
        .controller('ManageUsersCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth', 'store', 'TeacherService', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store, TeacherService) {
			...
			if(store.get('GetAllUsers') == undefined){
				TeacherService.GetAllUsers().then(function(response){
					...
					var response = store.get('GetAllUsers');
					var users = response;
					var allusers = [];
					var num = 0;
					for (var u = 0; u < users.length; u++) {
						if (users[u].is_admin == 1) {
							allusers.push({
								id:users[u].id,
								number: num += 1,
								name: users[u].name,
            ...
            $scope.adduserongo = function () {
                $window.location.href = $scope.tau;
            }

            $scope.edituserongo = function (uid) {
                $window.location.href = $scope.teu + "?" + uid;
            }

            $scope.deleteuserongo = function (uid) {
                TeacherService.DeleteUser(uid).then(function () {
                    ...
                    alert('Error occur when delete user, please contact admin!');
                });
            }
        }]);
})();