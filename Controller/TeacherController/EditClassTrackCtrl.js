(function () {
    ...
    angular
        ...
        .controller('EditClassTrackCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth', 'store', 'TeacherService','$location', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store, TeacherService,$location) {
			...
			if(store.get('GetDashboard') == undefined){
				TeacherService.GetDashboard().then(function(response){
					...
					for (var h = 0; h < houses.length; h++) {
						if (classid == houses[h].id) {
							classname = houses[h].house;
							for (var t = 0; t < houses[h].tracks.length; t++) {
								if (trackid == houses[h].tracks[t].id) {
									trackname = houses[h].tracks[t].track;
									tdescription = houses[h].tracks[t].description;
									lid = houses[h].tracks[t].level_id;
									fid = houses[h].tracks[t].field_id;
								}
							}
						}
					}
					...
					$scope.description = tdescription;
					$scope.levelid = lid;
					$scope.fieldid = fid;
					$scope.tname = trackname;
					$scope.trackname2 = trackname;
					$scope.description2 = tdescription;
					$scope.levelid2 = lid;
					$scope.fieldid2 = fid;
            ...
            $scope.reset = function () {
                var tn = $scope.trackname2;
                var td = $scope.description2;
                var li = $scope.levelid2;
                var fi = $scope.fieldid2;

                $scope.trackname = tn;
                $scope.description = td;
                $scope.levelid = li;
                $scope.fieldid = fi;
