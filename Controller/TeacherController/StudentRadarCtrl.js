    ...
    angular
        ...
        .controller('StudentRadarCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth', 'store', 'TeacherService', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store, TeacherService) {
            ...
            $scope.radarsearch = function () {                
				...
				var users = response;
				if ($scope.sname == undefined || $scope.sname == null) {
					alert('Student name field cannot be empty !!')
				}
				else {
					for (var u = 0; u < users.length; u++) {
						if (users[u].name == $scope.sname) {
							uid = users[u].id;
							student = users[u].firstname;
							$scope.rd = 1;
						}
					}
					$scope.student = student;
					if (uid == undefined) {
						alert('This student name is not available !');
					}
					else{
						TeacherService.GetOneUser(uid).then(function (responses) {
							var radarlabel = [];
							var radardata = [];
							var gfm = responses.data.getfieldmaxile;
							for (var g = 0; g < gfm.length; g++) {
								radarlabel.push(gfm[g].field);
								radardata.push(gfm[g].field_maxile);
							}
							$scope.radarlabel = radarlabel;
							...
							$('#exampleChartjsRadar').remove(); // this is my <canvas> element
							$('#chartrd').append('<canvas id="exampleChartjsRadar" height="400" width="600"></canvas>');
