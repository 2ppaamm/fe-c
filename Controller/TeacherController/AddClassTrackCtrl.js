(function () {
    ...
    angular
        ...
        .controller('AddClassTrackCtrl', ['$scope', '$state', '$window', '$http', 'aaNotify', '$rootScope', 'auth','store', 'TeacherService','$location', function ($scope, $state, $window, $http, aaNotify, $rootScope, auth, store, TeacherService,$location) {
			...
			if(store.get('GetDashboard') == undefined){
				TeacherService.GetDashboard().then(function(response){
					...
					for (var c = 0; c < classes.length; c++) {
						if (classid == classes[c].id) {
							classname = classes[c].house;
						}
					}
            ...
            $scope.addnewclasstrack = function () {
                var tn = $scope.trackname;
                var td = $scope.description;
                var lid = $scope.levelid;
                var fid = $scope.fieldid;
                TeacherService.PostCreateHousesTrack(classid, tn, td, lid, fid).then(function () {
                    alert('Successfull add new class track !');
					store.remove('GetDashboard');
					TeacherService.GetDashboard().then(function(response){
                    ...
                    if ($scope.trackname == undefined) {
                        ...
                        if ($scope.description == undefined) {
                            ...
                            if ($scope.levelid == undefined) {
                                alert('Level ID field is requried !');
                            }
                            else {
                                if ($scope.fieldid == undefined) {
                                    alert('Field ID field is requried!');
                                }
                                else {
